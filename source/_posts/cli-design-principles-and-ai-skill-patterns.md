...
这不是在说某一家平台不好，这几乎是所有平台 CLI 的第一版的样子。问题在于，这条路的终点是 AWS CLI v1——2500 个命令，`aws ec2 describe-instances` 的输出是一坨 JSON，用户的第一反应通常是 "怎么又要查文档"? 这么下去何时是个头？

然后 2024 年以来，事情变得更复杂了：Claude Code、Cursor、Copilot 开始直接调用 CLI。调用者不再只是人，还有会有"幻觉"的 LLM。"Agent call success rate"（Agent 调用成功率）成了一个新的质量指标。

本文想梳理几件事：

1. CLI 设计 40 年积累下来的核心原则是什么
2. AI 时代的 Built-in Skill 设计范式是什么
3. 以及当你要同时服务人类和 Agent 时，有没有可以参考的架构
...
这一行命令用了 6 个工具，每个工具只做一件事，但组合起来完成了复杂的日志分析。这就是是UNIX给咱们几十年的最优秀的设计——**stdout 是接口**，pipe 是集成机制。

但也同时意味着，要设计一个 CLI，其实要考虑好 CLI 的隐含合同：不要把无关的东西混进 stdout（那是给 pipe 下游的），进度、日志、警告都去 stderr；支持 `stdin` 输入（用 `-` 作为 stdin 别名）；`--json` 让输出可以被 `jq` 处理；exit code 的语义必须正确。
...
> 顺便吐槽一下 AWS CLI：它几乎是 pipeline 反面教材的集大成者。`aws ec2 describe-instances` 默认输出是完整 JSON 树，字段嵌套四五层，没有 `--query` 或 `jq` 基本不可读；同一个资源在不同子命令里输出结构还不一样。这不是设计，这是 2500 个命令各自为政的结果。说实话，不知道这背后又有多少亚麻人被折磨得夜夜失眠。
...
![LarkSuite CLI 三层架构 + AI Agent Skills](larksuite-three-layer.svg)
...
"Auto-generated from Lark OAPI metadata, curated through evaluation and quality gates" — LarkSuite 的描述是有意义的。不是所有 2500 个 API 都值得有一个命名命令，只有高频、语义清晰、适合脚本化的 API 才进入 Layer 2。这是信息架构决策，不是技术决策。
...
![SKILL.md 两层结构](skill-md-two-layer.svg)
...
对于 LarkSuite 的 Skill 来说，所有 19 个 Skill 都应该声明 `bins: ["lark-cli"]`——这是 Skill 能正常工作的前提，而不是可选项。

**`compatibility` 声明 Skill 的运行环境要求**，包括目标平台（Claude Code / Copilot / Cursor）、依赖的系统包、是否需要网络访问。跨平台发布 Skill 时，这个字段是分发的基础元信息。

**`allowed-tools` 是权限边界，不是工具列表。**

最小权限原则：只声明 Skill 实际需要的工具。一个只读日历的 Skill 不需要 `Bash` 权限。
...
**`disable-model-invocation: true` 是安全保险。**

设为 true 时，这个 Skill 从 LLM 的可用列表里消失，只能用户手动输入 `/skill-name` 触发。危险操作（清空数据、批量修改）应该用这个机制保护，不能让 LLM 自动触发。
...
├── references/       # 文档（Read 工具加载进 context，tokens消耗大）
└── assets/           # 模板/二进制（仅路径引用，tokens消耗小）
```
...
最后也想重申一句：没有任何东西是万能的，都会有其对应的适应场景，LarkSuite CLI的三层架构也是一样的。它的代价是用户的认知负担——用户需要理解什么时候用 Shortcuts、什么时候用 API Commands、什么时候用 Raw API。这个架构适合业务域足够复杂（LarkSuite 有 11 个业务域）、同时服务人类和 Agent 的平台。如果你的平台 CLI 场景相对单一，过度分层只会增加维护成本。

最后的最后，记住一句，在Agent时代，不要等架构完整了再动手。从一个 Shortcut 开始，从一个清晰的 Skill description 开始——这两件事做好了，比设计一套完整的三层体系更有价值。等你真的做大了，再考虑分层架构也不迟。**别让完美的架构成为你交付的敌人。**
...
- [Extend Claude with skills - Claude Code Docs](https://docs.claude.com/code/agents/skills/extend-claude)
- [Equipping agents for the real world with Agent Skills](https://aws.amazon.com/blogs/machine-learning/equipping-agents-for-the-real-world-with-agent-skills/)
