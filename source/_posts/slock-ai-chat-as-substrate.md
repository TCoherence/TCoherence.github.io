...
1. API key 也是用户的（RC 自己承认），不能按 token 收费。
1. 订阅？需要 retention，retention 靠什么？目前看 retention 来自"agent 动力学"产生的 stickiness，但还没有数据支撑。
1. 企业版？企业要求 sandbox，slock 当前形态完全不满足；同时 retrofit 路线（Teams / 飞书）已经在企业用户基础里了。
