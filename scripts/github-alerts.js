...
/**
 * Convert GitHub-flavored alert blockquotes into styled callouts.
 *
 *   > [!NOTE]
 *   > body...
 *
 * markdown-it-plus renders this as a plain <blockquote>; this filter
 * post-processes the HTML to wrap it in <div class="markdown-alert
 * markdown-alert-{type}"> matching GitHub's own DOM, then styles.styl
 * gives it the colored left-border + title row.
 *
...
const PATTERN = /<blockquote>\s*<p>\s*\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\s*(?:<br\s*\/?>\s*)?([\s\S]*?)<\/p>\s*<\/blockquote>/g;

function transform(html) {
  if (typeof html !== 'string' || !html.includes('[!')) return html;
  return html.replace(PATTERN, (_, type, body) => {
    const lc = type.toLowerCase();
    return (
      `<div class="markdown-alert markdown-alert-${lc}">` +
      `<p class="markdown-alert-title">${ICONS[lc]}<span>${LABELS[lc]}</span></p>` +
      `<p>${body}</p>` +
      `</div>`
    );
  });
...
hexo.extend.filter.register('after_render:html', transform);
